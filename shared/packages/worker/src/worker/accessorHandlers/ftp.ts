...
import { FTPClient, FTPClientBase, FTPOptions } from './lib/FTPClient'
...
export class FTPAccessorHandle<Metadata> extends GenericAccessorHandle<Metadata> {
	...
	async putPackageStream(sourceStream: NodeJS.ReadableStream): Promise<PutPackageHandler> {
		...
		pResponse
			...
			.catch((err) => {
				streamWrapper.emit('error', new Error(`FTP upload threw: ${err}`))
			})
	...
	async finalizePackage(operation: PackageOperation): Promise<void> {
		...
		if (this.workOptions.useTemporaryFilePath) {
			...
			if (await ftp.removeFileIfExists(this.temporaryFilePath)) {
	...
	async fetchMetadata(): Promise<Metadata | undefined> {
		...
		// The file exists
		if (await ftp.fileExists(this.metadataPath)) {
	...
	private get basePath(): string {
		return this.accessor.basePath ?? '/'
	}
	get filePath(): string {
		if (this.content.onlyContainerAccess) throw new Error('onlyContainerAccess is set!')
		const filePath = this._getFilePath()
		if (!filePath) throw new Error(`GTPAccessorHandle: path not set!`)
		return filePath
	}
	...
	private async prepareFTPClient(): Promise<FTPClientBase> {
		...
		const options: FTPOptions = {
			...
			username,
			password,
			allowAnyCertificate: this.accessor.allowAnyCertificate ?? false,
		}
		...
		if (!cachedClient) {
			...
			if (serverType === 'ftp' || serverType === 'ftps') {
				this.worker.accessorCache
				cachedClient = new FTPClient(options)
		...
		if (cachedClient) {
			...
			throw new Error(`FTPAccessorHandle: Could not create FTP client for ${serverType}`)
		}
	}
	...
	private _getFilePath(): string | undefined {
		return this.accessor.path || this.content.path
	}
	...
	async fileExists(fullPath: string): Promise<boolean> {
		const ftp = await this.prepareFTPClient()
		const exists = await ftp.fileExists(fullPath)
