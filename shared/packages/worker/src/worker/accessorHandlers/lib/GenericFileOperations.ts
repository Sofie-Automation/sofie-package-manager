import { Expectation, Reason, stringifyError } from '@sofie-package-manager/api'
import { JSONWriteFilesHandler } from './json-write-file'
...
export class GenericFileOperationsHandler {
	private jsonWriter: JSONWriteFilesHandler
	constructor(private fileHandler: GenericFileHandler, private workOptions: Expectation.WorkOptions.RemoveDelay) {
		this.jsonWriter = new JSONWriteFilesHandler(fileHandler)
	}
	...
	async handleRemovePackage(filePath: string, packageName: string, reason: string): Promise<void> {
		if (this.workOptions.removeDelay) {
			...
			if (await this.fileHandler.unlinkIfExists(filePath)) {
	...
	private async updatePackagesToRemove(
		...
		if (this.fileHandler.storePackagesToRemove && this.fileHandler.getPackagesToRemove) {
			...
			await this.fileHandler.storePackagesToRemove(newList)
			return
		}
		...
		try {
			await this.jsonWriter.updateJSONFileBatch<DelayPackageRemovalEntry[]>(
				...
				},
				{
					...
					logError: (error) => this.fileHandler.logger.error(stringifyError(error)),
					logWarning: (message) => this.fileHandler.logger.warn(message),
				}
			)
		} catch (e) {
			// Not much we can do about it..
			// Log and continue:
			this.fileHandler.logger.error(stringifyError(e))
		}
	}
}
